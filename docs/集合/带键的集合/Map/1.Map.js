...
//3. Map.prototype.entries() 返回一个新的 map 迭代器对象,迭代元素[key， value]，
//4. Map.prototype.forEach()
//5. Map.prototype.has() 方法返回一个布尔值，指示此 Map 中是否存在具有指定键的元素
//6. Map.prototype.keys() 返回一个新的 map 迭代器对象，迭代元素 key
*/
...
for( const [key, value] of fooMapIterator){
...
for( const key of fooMapIteratorKeys){
  console.log(`${key}`);
}
